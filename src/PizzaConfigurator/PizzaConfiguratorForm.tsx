...
export const PizzaConfiguratorForm = ({
  ...
  const updatePizzaSize = (event: ChangeEvent<HTMLInputElement>) => {
    setPizzaSize(parseInt(event.target.value, 10));
  };

  const updateDoughType = (event: ChangeEvent<HTMLInputElement>) => {
    setDoughType(event.target.value);
  };

  const updateSauce = (event: ChangeEvent<HTMLInputElement>) => {
